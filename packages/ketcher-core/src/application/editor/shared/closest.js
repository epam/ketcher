...
function findClosestSGroup(restruct, pos) {
  ...
  restruct.molecule.sgroups.forEach((sg, sgid) => {
    if (sg.isContracted()) {
      const masterAtomId = sg.getContractedGroupMasterAtomId()
      const masterAtom = restruct.molecule.atoms.get(masterAtomId)
      const masterAtomPp = masterAtom.pp
      const d = sg.bracketDir
      const n = d.rotateSC(1, 0)
      const pg = new Vec2(Vec2.dot(pos, d), Vec2.dot(pos, n))
      const shift = new Vec2(0.625, 0.625)
      const box = {
        p0: Vec2.diff(masterAtomPp, shift),
        p1: Vec2.sum(masterAtomPp, shift)
      }
