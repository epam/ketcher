...
ReStruct.prototype.showText = function () {
  this.texts.forEach((text, id) => {
    const path = text.draw(this.render, id)
    if (path) this.addReObjectPath('data', text.visel, path, null, true)
    // TODO fragment selection & highlighting
  })
}
...
function isSelectionSvgObjectExists(item) {
  return (
    (item.selectionPlate !== null && !item.selectionPlate.removed) ||
    (Array.isArray(item.selectionPlate) && !item.selectionPlate[0].removed)
  )
}
