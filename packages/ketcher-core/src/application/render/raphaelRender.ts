...
function calcExtend(
  ...
  console.log('calcExtend: ', scaledSz, x0, y0, x1, y1)
  let ex = x0 < 0 ? -x0 : 0
  let ey = y0 < 0 ? -y0 : 0

  if (scaledSz.x < x1 * scale) {
    console.log('calcExtend', scaledSz.x, x1)
    ex += x1 * scale - scaledSz.x
  }
  if (scaledSz.y < y1 * scale) {
    ey += y1 * scale - scaledSz.y
  }
  // if (scaledSz.x < x1) {
  // console.log('calcExtend', scaledSz.x, x1)
  // ex += x1 - scaledSz.x
  ...
  // ey += y1 - scaledSz.y
  // }
  console.log('calcExtend result', new Vec2(ex, ey))
