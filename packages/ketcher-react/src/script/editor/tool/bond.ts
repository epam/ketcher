...
class BondTool {
  ...
  mousedown(event) {
    ...
    this.dragCtx = {
      ...
      item: this.editor.findItem(event, ['atoms', 'bonds'], null)
    }
  ...
  mousemove(event) {
    ...
    if (this.dragCtx) {
      ...
      if (!('item' in dragCtx) || dragCtx.item.map === 'atoms') {
        ...
        if ('item' in dragCtx && dragCtx.item.map === 'atoms') {
          ...
          const fGroupAtoms: any =
            fGroup && SGroup.getAtoms(molecule, fGroup.item)
          if (endAtom && fGroup && endAtom.id !== fGroupAtoms?.[0]) {
            this.editor.event.removeFG.dispatch({ fgIds: [fGroupId] })
            endAtom = null
          }
          if (endAtom && fGroup && endAtom.id === fGroupAtoms?.[0]) {
            const atomNeighbours: any = molecule.atomGetNeighbors(endAtom.id)
          ...
          endAtom = editor.findItem(event, ['atoms'], null)
          const atomResult: Array<any> = []
          const result: Array<any> = []
          if (
            endAtom &&
    ...
    this.editor.hover(this.editor.findItem(event, ['atoms', 'bonds'], null))
    return true
  }
