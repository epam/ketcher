...
Supported formats are: *.mol (v3000), *.HELM and text files with MOL or HELM notation inside (*.txt , .*doc, .docx, .rtf). 
...
| 3    |                                 |                                                | clicking on the Open icon <br/>OR <br/>using the shortcut Ctrl + O<br/>AND<br/>way of adding file is selected | Ketcher Polymer Editor checks that structure opened is a polymer one<br>AND <br/>Ketcher provides the ability to add structure to Canvas<br/>OR<br/>Open structure as a new project |
| 3.1  |                                 |                                                | conditions from the no3  are fulfilled<br/>AND<br/>'Add to Canvas' option was selected | Canvas in the same state as it was before adding new structure <br/>AND<br/>Ketcher creates the new structure which is moved on canvas by the user's cursor. The first cursor click determines the location of the structure on the canvas |
| 3.2  |                                 |                                                | conditions from the no3  are fulfilled<br/>AND<br/>'Open as New project' option was selected | New canvas is opening and Ketcher creates the structure in the upper left corner. |
