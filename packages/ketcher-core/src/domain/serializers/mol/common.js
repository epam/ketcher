...
function parseCTab(
  ...
  if (version === 'V3000') {
    throw new Error(
      'V3000 molfile parsing is delegated to Indigo and is not supported by MolSerializer.',
    );
  }
...
function parseRxn(
  ...
  if (split.length > 1 && split[1] === 'V3000') {
    throw new Error(
      'V3000 RXN parsing is delegated to Indigo and is not supported by MolSerializer.',
    );
  }
