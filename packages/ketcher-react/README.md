...
- **Browser runtime**: a modern browser with native `fetch`, `URLSearchParams`, `IntersectionObserver`, `Element.closest`, and async/generator support. `ketcher-react` no longer loads these polyfills automatically; add them in your app only if you intentionally target older browsers.
