...
Struct.prototype.mergeInto = function (cp, atomSet, bondSet, dropRxnSymbols, keepAllRGroups, aidMap) { // eslint-disable-line max-params, max-statements
	...
	this.sgroups.forEach(sg => {
		...
		sg.atoms.forEach(aid => cp.atoms.get(aid).sgs.add(id));
...
Struct.prototype.initHalfBonds = function () {
	...
	this.bonds.forEach((bond, bid) => this.bondInitHalfBonds(bid, bond));
};
...
Struct.prototype.sortNeighbors = function (list) {
	if (!list)
		this.atoms.forEach((atom, aid) => this.atomSortNeighbors(aid));
	else
		list.forEach(aid => this.atomSortNeighbors(aid));
};
...
Struct.prototype.updateHalfBonds = function (list) {
	if (!list)
		this.atoms.forEach((atom, aid) => this.atomUpdateHalfBonds(aid));
	else
		list.forEach(aid => this.atomUpdateHalfBonds(aid));
};
...
Struct.prototype.setImplicitHydrogen = function (list) {
	...
	if (!list)
		this.atoms.forEach(aid => this.calcImplicitHydrogen(aid));
	else
