...
`setMolecule(structure: string, x?: number, y?: number): Promise<void>` – draws passed structure on the canvas. Before drawing passed structure, current structure is removed.
Parameters: `structure: string`. Structure is a string in any supported format. `x: number`. X axis position on canvas (optional). `y: number`. Y axis position on canvas (optional)
...
Parameters: `structure: string`. Structure is a string in any supported format. X axis position on canvas (optional). `y: number`. Y axis position on canvas (optional)
