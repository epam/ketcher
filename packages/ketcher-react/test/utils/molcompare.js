...
function arraysEquals(sArray, rArray, eqFunc) {
  return (
    ...
    (sArray.length === rArray.length &&
      sArray.every((elem, index) =>
        eqFunc ? eqFunc(elem, rArray[index]) : elem === rArray[index]
      ))
  )
}
...
function compareFile(source, dest, options) {
  ...
  for (var i = 0; i < sourceStr.length; i++) {
    ...
    const numberEquals = arraysEquals(
      sDataSplitted.numbers,
      rDataSplitted.numbers,
      (x, y) => epsEqu(x, y, options.eps)
    )
    const characterEquals = arraysEquals(
      sDataSplitted.characters,
      rDataSplitted.characters
    )
...
function compareRaw(source, dest, options) {
  ...
  for (let i = 0; i < sDataMols.length - 1; i++) {
    ...
    newSdata = sDataMols[i].substring(
      6 + (sV3000DataIndex === -1 ? sV2000DataIndex : sV3000DataIndex)
    )
    newRdata = rDataMols[i].substring(
      6 + (rV3000DataIndex === -1 ? rV2000DataIndex : rV3000DataIndex)
    )

    if (!compareFile(newSdata, newRdata, options)) return false
  }
  return true
}
...
function compareObjects(source, dest, options, parentKey) {
  ...
  if (typeof source !== 'object') return source === dest
