...
function createStyle(style) {
  return Object.keys(style).reduce((str, prop) => {
    ...
    if (typeof value == 'object') return str
    if (typeof value == 'number' || typeof value == 'string') value += 'px'
...
function symbolRender(symbolStr) {
  const symbol = new DOMParser().parseFromString(symbolStr, 'application/xml')
    .firstElementChild

  let svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
  for (let attr of symbol.attributes) svg.setAttribute(attr.name, attr.value)
