...
function fromTemplateOnBond(restruct, bid, template, flip) {
	...
	tmpl.bonds.forEach(bond => {
		const existId = struct.findBondId(atomsMap.get(bond.begin), atomsMap.get(bond.end));
		if (!existId)
			action.addOp(new op.BondAdd(atomsMap.get(bond.begin), atomsMap.get(bond.end), bond).perform(restruct));
		else
