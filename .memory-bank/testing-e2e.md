...
> **Read when:** writing, changing, running or triaging a spec in `ketcher-autotests/`.
> **Skip when:** unit tests, type checks or lint — [testing.md](./testing.md) covers those.

All E2E tests live in `ketcher-autotests/`, a separate npm workspace: Playwright, Chromium only
(headless), config `ketcher-autotests/playwright.config.ts`, spec files `*.spec.ts` under
`tests/specs/`. The `@playwright/test` version is the one in the root `package.json`; it must equal
the tag of the `mcr.microsoft.com/playwright` image used by CI and by `ketcher-autotests/Dockerfile`,
because the Linux baselines are rendered by that image.
...
| `@utils` / `@utils/*`         | `tests/utils` (the folder; `index.ts` is its barrel) |
| `@tests` / `@tests/*`         | `tests` / `tests/*`                                  |
| `@constants` / `@constants/*` | `constants/`                                         |
| `@app/*`                      | the package root                                     |

Always use these aliases. Never use relative `../../` paths from test files. Spec files import
`test`, `expect`, `Page` and `Locator` from `@fixtures`; page-object files are the one exception and
import `Page`/`Locator` from `@playwright/test`.

### Traps worth knowing before you rely on a helper

- A run with `--update-snapshots` (or `GENERATE_DATA=true`, both set by `npm run docker:update`)
  regenerates **expected data files as well as PNGs** — a snapshot refresh can silently bless a
  broken export. Review every changed `*-expected.*` in the diff.
- `verifyFileExport` filters metadata lines containing `-INDIGO-`, `$DATM`, `$MDL` or `Ketcher`.
  Passing `metaDataIndexes` replaces that filter with line numbers instead of adding to it.
- `takePageScreenshot` accepts `hideMonomerPreview` and `hideMacromoleculeEditorScrollBars` but
  ignores them; only `takeEditorScreenshot` and `takeElementScreenshot` honour them.
- `waitForRender` only warns when the `renderComplete` event never arrives (and the warning needs
  `ENABLE_CUSTOM_WARNINGS`), so a broken wait degrades into a 250 ms sleep.
- Tags (`@IncorrectResultBecauseOfBug`, `@NeedToBeUpdated`, `@FlakyTest`) are documentation: no
  config or CI job filters on them. `IGNORE_UNSTABLE_TESTS` is set by CI and read by nothing.
- `markResetToDefaultState` / `processResetToDefaultState` survive in two spec files only, and the
  processing half returns early unless the test passed — prefer inline teardown.
...
1. Build what CI builds and serve it, from the repository root:

   ```sh
   npm ci
   npm run build:packages && npm run build:example:standalone
   npm run serve:standalone     # example/dist/standalone on 127.0.0.1:4002
   ```

2. In `ketcher-autotests/`, copy `.env.example` to `.env` (it sets `KETCHER_URL=http://localhost:4002`
   and `MODE="standalone"`) and install the browser once: `npx playwright install chromium`.
   Without `KETCHER_URL` and `MODE` the config falls back to the Vite dev server on port 5173, and
   popup tests then open the main page instead of `/popup.html`.

### Run Commands (from `ketcher-autotests/`)

```sh
npx playwright test "tests/specs/Chromium-popup/<path>.spec.ts" --project=chromium-popup
npx playwright test "tests/specs/<path>.spec.ts:<line>" --project=chromium
npm run test:debug -- "<path>.spec.ts:<line>"   # Playwright inspector
npm run report                                   # the last HTML report
```

Quote spec paths: one top-level folder is `Structure-Creating-&-Editing`. The config always writes
`results.json`; the `ketcher-e2e-run` skill summarises it, which is far cheaper than reading the raw
reporter output.

A run on Windows or macOS compares screenshots with local `-win32`/`-darwin` baselines, which are
git-ignored and created on first run — it cannot confirm a visual result. Use Docker for that.

### Docker (snapshot generation)

Snapshots are OS-specific. Always generate and commit Linux snapshots via Docker. The scripts call
the `docker-compose` command and need `.env` plus a host build of `example/dist/standalone`:

```sh
npm run docker:build                             # build the image, install inside it
npm run docker:test -- "<spec>[:line]"           # project chromium
npm run docker:test-popup -- "<spec>[:line]"     # project chromium-popup
npm run docker:update -- "<spec>[:line]"         # regenerate snapshots and expected files (both projects)
npm run docker:update-popup -- "<spec>[:line]"   # the same for chromium-popup
```
