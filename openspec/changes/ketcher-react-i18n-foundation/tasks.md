...
- [x] 0.10 **Regression found post-Section-9 and fixed:** the `<div dir={i18n.dir()}>` wrapper added in 0.5 had no sizing CSS. `Editor.tsx`'s two `.editorsWrapper` panes depend on a `height: 100%` cascade from an explicit-height ancestor; the unstyled wrapper broke that chain, collapsing the left/right tool panels and canvas to zero height (editor rendered as just the top toolbar) in both molecules and macromolecules mode. The 0.8 visual check at the time didn't catch it — likely because the app was checked at a moment/viewport where the collapse wasn't obvious, or the check focused on text rather than layout. Fixed by adding a `.root { height: 100%; width: 100%; }` class to the wrapper — **Committed:** `c623b56611` on `4384-language`. Verified live by reverting the fix to reproduce the collapsed layout, then confirming the fix restores it in both editor modes.
...
- [x] 9.5 Playwright E2E coverage: **skipped by explicit decision** — asked whether to proceed now, defer to a follow-up change, or skip; user chose to skip. Rationale holds: this change ships English-only, existing E2E text-based assertions are expected to pass unmodified, and selector/testid behavior was explicitly kept identical wherever extraction touched it (see Section 6's `MeasureInput`/`Select`/`Accordion` testid-preservation work). No new E2E coverage added in this change.
- [x] 9.6 **Committed:** `32ce5ddcba` on `4384-language` (covers 9.1-9.3; 9.5 skipped by explicit decision, see above)
