...
rnd.MolData.prototype.drawBondAromatic = function (hb1, hb2, bond)
{
    ...
    (shift > 0 ? l1 : l2).attr({'stroke-dasharray':'- '});
...
rnd.MolData.prototype.showAtomHighlighting = function (aid, atom, visible)
{
	...
	if (visible) {
		if (!exists) {
			...
			if (rnd.DEBUG)
				atom.highlighting.attr({'fill':'#AAA'});
			this.addAtomPath('highlighting', aid, atom.highlighting);
		}
		if (rnd.DEBUG)
			atom.highlighting.attr({'stroke':'#0c0'});
		else
		    atom.highlighting.show();
	} else {
		if (exists) {
			if (rnd.DEBUG)
				atom.highlighting.attr({'stroke':'none'});
			else
    ...
    for (var aid in this.atomsChanged) {
        ...
        if (opt.showAtomIds) {
            ...
            index.path = paper.text(atom.ps.x, atom.ps.y, index.text)
                .attr({'font' : settings.font, 'font-size' : settings.fontszsub, 'fill' : '#070'});
        ...
        if (atom.showLabel)
        {
            ...
            label.path = paper.text(atom.ps.x, atom.ps.y, label.text)
                .attr({'font' : settings.font, 'font-size' : settings.fontsz, 'fill' : color});
			...
			if (isHydrogen && implh > 0) {
				...
				hydroIndex.path =
					paper.text(atom.ps.x, atom.ps.y, hydroIndex.text)
						.attr({'font' : settings.font, 'font-size' : settings.fontszsub, 'fill' : color});
	...
	if (visible) {
		if (!exists) {
			...
			bond.highlighting = paper
				...
				.attr(styles.highlightStyle);
			if (rnd.DEBUG)
				bond.highlighting.attr({'fill':'#AAA'});
			this.addBondPath('highlighting', bid, bond.highlighting);
		}
		if (rnd.DEBUG)
			bond.highlighting.attr({'stroke':'#0c0'});
		else
			bond.highlighting.show();
	} else {
		if (exists) {
			if (rnd.DEBUG)
				bond.highlighting.attr({'stroke':'none'});
			else
