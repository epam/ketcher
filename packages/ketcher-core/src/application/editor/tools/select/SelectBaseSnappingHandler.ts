...
export class SelectBaseSnappingHandler {
  constructor(private readonly editor: CoreEditor) {}

  public applyMovementWithSnapping(
    event: MouseEvent,
    movementDelta: Vec2,
  ...
  static calculateGroupCenterSnapPosition(
    ...
    for (let i = 0; i < connectedMonomers.length; i++) {
      for (let j = i + 1; j < connectedMonomers.length; j++) {
        ...
        if (
          SelectBaseSnappingHandler.needApplyGroupCenterSnapForOneAxis(
  ...
  static checkMonomersAlignment(
    ...
    const alignment = SelectBaseSnappingHandler.determineAlignment(
      firstMonomer.center,
  ...
  static findRemainingAlignedMonomers(
    ...
    const traverse = (currentMonomer: BaseMonomer) => {
      const nextMonomers = SelectBaseSnappingHandler.getNextMonomers(
        currentMonomer,
        visitedMonomers,
      );
      for (const nextMonomer of nextMonomers) {
        ...
        if (
          SelectBaseSnappingHandler.checkMonomersAlignment(
            currentMonomer,
            nextMonomer,
            alignment,
            distance,
  ...
  static calculateSideDistanceSnap(
    ...
    const additionalAlignedMonomers =
      SelectBaseSnappingHandler.findRemainingAlignedMonomers(
        monomerForAlignment,
        alignedMonomers,
        alignment,
        snapDistance,
  ...
  static calculateDistanceSnap(
    ...
    if (!secondShortestBond) {
      return SelectBaseSnappingHandler.calculateSideDistanceSnap(
        cursorPosition,
        initialMonomer,
        connectedMonomer,
      );
    ...
    return SelectBaseSnappingHandler.calculateInBetweenDistanceSnap(
      cursorPosition,
      initialMonomer,
      connectedMonomer,
      secondConnectedMonomer,
  ...
  private tryToSnap(event: MouseEvent, movementDelta: Vec2) {
    ...
    > = externalConnectionsToSelection.map(
      ({ monomerFromSelection, monomerConnectedToSelection, bond }) => {
        ...
        const {
          ...
          alignment,
          alignedMonomers,
        } = SelectBaseSnappingHandler.calculateDistanceSnap(
        ...
        const { angleSnapPosition, snappedAngleRad } =
          SelectBaseSnappingHandler.calculateAngleSnap(
        ...
        const { bondLengthSnapPosition } =
          SelectBaseSnappingHandler.calculateBondLengthSnap(
            selectedMonomer.position,
            connectedMonomer.position,
            snappedAngleRad,
          );
    ...
    if (
      ...
      const groupCenterSnapResult =
        SelectBaseSnappingHandler.calculateGroupCenterSnapPosition(
          selectedMonomers,
          connectedMonomers,
          movementDelta,
        );
