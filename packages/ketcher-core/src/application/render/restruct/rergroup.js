...
class ReRGroup extends ReObject {
  ...
  draw(render, options) {
    ...
    const ret = { data: [] };
    const p0 = Scale.protoToCanvas(bb.p0, options);
    const p1 = Scale.protoToCanvas(bb.p1, options);
  ...
  _draw(render, rgid, attrs) {
    ...
    const p0 = Scale.protoToCanvas(bb.p0, render.options);
    const p1 = Scale.protoToCanvas(bb.p1, render.options);
    return render.paper
...
function rGroupdrawBrackets(set, render, bb, d) {
  d = Scale.protoToCanvas(d || new Vec2(1, 0), render.options);
  ...
  const leftBracket = draw.bracket(
    ...
    Scale.protoToCanvas(new Vec2(bb.p0.x, cy), render.options),
    bracketWidth,
    bracketHeight,
    render.options,
  );
  ...
  const rightBracket = draw.bracket(
    ...
    d,
    d.rotateSC(1, 0),
    Scale.protoToCanvas(new Vec2(bb.p1.x, cy), render.options),
    bracketWidth,
    bracketHeight,
    render.options,
  );
