...
import { AttachmentPointOption, AttachmentPointAtom } from '@tests/pages/molecules/canvas/createMonomer/constants/editConnectionPointPopup/Constants';
...
test.describe('Autotests: Attachment points editing dropdown logic in monomer creation wizard', () => {
  ...
  test('Case 1 - Verify dropdown shows only H and OH when current LGA is H', async () => {
    ...
    await pasteFromClipboardAndOpenAsNewProject(page, 'CCCCC'); // Simple carbon chain
    ...
    const r1AtomDropdown = createMonomerDialog.getAttachmentPointAtomCombobox(AttachmentPointOption.R1);
  ...
  test('Case 3 - Verify dropdown shows H, OH, then other atom when current LGA is not H or OH', async () => {
    ...
    await pasteFromClipboardAndOpenAsNewProject(page, 'CCCNCC'); // Chain with nitrogen
    ...
    expect(optionTexts[2]).toMatch(/N/); // Should contain N (could be NH₂, NH₃, etc.)
  ...
  test('Case 4 - Verify LGA replacement works without changing position', async () => {
    ...
    // Get initial position of the leaving atom (should be H initially)
    const initialLeavingAtoms = page.locator('[data-testid*="H"]');
    const initialCount = await initialLeavingAtoms.count();
  ...
  test('Case 5 - Verify currently selected LGA is visually indicated in dropdown', async () => {
    ...
    const selectedOption = page.locator('[role="option"][aria-selected="true"], [role="option"].Mui-selected, [role="option"].selected');
    ...
    await r1AtomDropdown.click();
    const newSelectedOption = page.locator('[role="option"][aria-selected="true"], [role="option"].Mui-selected, [role="option"].selected');
  ...
  test('Case 6 - Verify dropdown behavior with multiple attachment points', async () => {
    ...
    await pasteFromClipboardAndOpenAsNewProject(page, 'CCCCCC'); // Six carbon chain
